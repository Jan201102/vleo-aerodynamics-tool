function stiffness = calculate_aerodynamic_stiffness(method, delta, axis_dir_B, component, bodies, bodies_rotation_angles__rad, model, lut)
...
%   lut: string specifying the path to the look-up table for aerodynamic coefficients
%
% Outputs:
...
%   calculated at the specified rotation axis and component
    arguments
        ...
        lut (1,1) string {mustBeFile};  % Path to the look-up table for aerodynamic coefficients
    end

    switch method
        case 'central'
            % Central difference: f'(x) ≈ (f(x+h) - f(x-h)) / (2h)
            torque_plus = compute_aerodynamics_at_angle(delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_minus = compute_aerodynamics_at_angle(-delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            stiffness = (torque_plus(component) - torque_minus(component)) / (2 * delta);
            
        case 'five_point_stencil'
            % Five-point stencil: f'(x) ≈ (-f(x+2h) + 8f(x+h) - 8f(x-h) + f(x-2h)) / (12h)
            torque_1 = compute_aerodynamics_at_angle(-2*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_2 = compute_aerodynamics_at_angle(-delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_3 = compute_aerodynamics_at_angle(delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_4 = compute_aerodynamics_at_angle(2*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            stiffness = (-torque_4(component) + 8*torque_3(component) - 8*torque_2(component) + torque_1(component)) / (12 * delta);
            
        case 'seven_point_stencil'
            % Seven-point stencil: f'(x) ≈ (-f(x-3h) + 9f(x-2h) - 45f(x-h) + 45f(x+h)  9f(x+2h) + f(x+3h)) / (60h)
            torque_1 = compute_aerodynamics_at_angle(-3*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_2 = compute_aerodynamics_at_angle(-2*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_3 = compute_aerodynamics_at_angle(-delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_4 = compute_aerodynamics_at_angle(delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_5 = compute_aerodynamics_at_angle(2*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_6 = compute_aerodynamics_at_angle(3*delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            stiffness = (-torque_1(component) + 9*torque_2(component) - 45*torque_3(component) + 45*torque_4(component) - 9*torque_5(component) + torque_6(component)) / (60 * delta);
            
        case 'forward'
            % Forward difference: f'(x) ≈ (f(x+h) - f(x)) / h
            torque_0 = compute_aerodynamics_at_angle(0, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_plus = compute_aerodynamics_at_angle(delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            stiffness = (torque_plus(component) - torque_0(component)) / delta;
            
        case 'backward'
            % Backward difference: f'(x) ≈ (f(x) - f(x-h)) / h
            torque_0 = compute_aerodynamics_at_angle(0, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            torque_minus = compute_aerodynamics_at_angle(-delta, axis_dir_B, bodies, bodies_rotation_angles__rad, model, lut);
            stiffness = (torque_0(component) - torque_minus(component)) / delta;
