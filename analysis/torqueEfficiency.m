...
% Analyze the relationship between torque generation and drag penalty
% for shuttlecock geometry comparing Sentman and IRS models
...
aerodynamic_torques_B_B__Nm = aerodynamic_forces_B__N;
...
for i = 1:num_angles
    for model = 1:2
        ...
        [aerodynamic_force_B__N(:,i,model), ...
         ...
         aerodynamic_torques_B_B__Nm(:,:,i,model)] = ...
            vleoAerodynamics(...
...
hold on;
grid on;
plot(aerodynamic_torque_B_B__Nm(2,:,1), -aerodynamic_force_B__N(1,:,1), 'b', 'DisplayName', ...
     sprintf('Sentman \\alpha_E = %.4f', environment_definitions.energy_accommodation));
plot(aerodynamic_torque_B_B__Nm(2,:,2), -aerodynamic_force_B__N(1,:,2), 'r', 'DisplayName', 'IRS Model');
xlabel('Pitch Torque [Nm]');
ylabel('Drag Force [N]');
title('Torque Efficiency: Drag vs Pitch Torque');
